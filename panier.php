...
<body>
  ...
  <div class="panier-content">
    <?php if (!empty($jeux_in_cart)): ?>
      <table>
        ...
        <tbody>
          ...
          <?php foreach ($jeux_in_cart as $jeux): ?>
            ...
            <?php
            // Calculer le total et mettre à jour la session
            $total += floatval($jeux['prix']) * intval($cart_items[$jeux['id']]);
            $_SESSION['total'] = $total; // Mettre à jour la session
            ?>
      ...
      <div class="total">
        <p id="prix">Total: €<?php echo number_format($_SESSION['total'], 2); ?></p> <!-- Afficher le total depuis la session -->
